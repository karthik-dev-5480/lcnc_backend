...
public class DataController {
    ...
    public ResponseEntity<String> syncById(@PathVariable Long tableId) {
        try {
            dataService.SyncSchema(tableId);
            return ResponseEntity.ok("Physical table schema synced successfully for ID: " + tableId);
        } catch (Exception e) {
    ...
    public ResponseEntity<DataRelationshipResponse> createRelationship(@RequestBody DataRelationshipRequest request) {
    	System.out.println(request.getSourceColumnId());
    	System.out.println(request.getTargetColumnId());
    	System.out.println(request.getTargetTableId());
    	System.out.println(request.getSourceTableId());
